...
class SalesOrderSaveAfterObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        try {
            ...
            if ($origStatus === null || $origStatus === $order->getStatus()) {
                return;
            }
            $this->dispatcher->dispatch('order.updated', $order);
        } catch (Exception $e) {
            $this->logger->error('UCP observer error (order.updated)', ['exception' => $e->getMessage()]);
        }
    }
}
